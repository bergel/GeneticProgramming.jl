...
@testset "REL example" begin
    function gp_infix_print(n::GPNode, res::Vector{String})
        gp_print(gp_children(n)[1], res)
        push!(res, " ")
    ...
    function accessdata_print(n::GPNode, res::Vector{String})
        push!(res, "data(")
        gp_print(gp_children(n)[1], res)
        push!(res, ")")
    end
    ...
    rules = [
        :expr => [Atom(:accessdata, ()->"data", accessdata_print), :variable],
        :expr => [:expr, Atom(:and, ()->"and", gp_infix_print), :expr],
        :expr => [:condition],

        :variable => [Atom(:variable, ()->"x")],
        :variable => [Atom(:variable, ()->"y")],
        :condition => [:variable, Atom(:<, ()->"<", gp_infix_print), :number],
        :condition => [:variable, Atom(:>, ()->">", gp_infix_print), :number],
        :number => [Atom(:number, ()->rand(-10:10))]
    ]
    gp_config = GPConfig(rules; maximum_width=10)

    #=
    Template of REL program:
        def data(x) = range(-10, 10, 1, x)
        def myrel(x,y) = ...
        def output = myrel
    ...
    E.g.:
        def data(x) = range(-10, 10, 1, x)
        def myrel(x,y) = data(x) and data(y) and x + 1 = 10
        def output = myrel
    =#

    @test gp_print(build_individual(gp_config, :expr)) == "hello"
end
